...
try:
    from trader import (init_trader, execute_buys, start_monitor, stop_monitor,
                        _sync_positions_from_wallet, has_open_position, is_in_cooldown,
                        DB_PATH)
    _HAS_TRADER = True
...
def load_config() -> dict:
    ...
    if CONFIG_PATH.exists():
        ...
        with open(CONFIG_EXAMPLE_PATH, "r", encoding="utf-8") as f:
            config = json.load(f)
    if "BSCSCAN_API_KEY" not in config or not config["BSCSCAN_API_KEY"]:
        import os
        env_key = os.environ.get("BSCSCAN_API_KEY", "")
        if env_key:
            config["bscscan_api_key"] = env_key
    return config
...
# 要求: 阳线 + 成交额≥前6根之和(不足6根则归一化到6根) + 涨幅≤50% + 增量≥500u
# 注意: 仅限币龄<24h代币 (volume24h是24h累计值, >24h差分不准确)
BONUS_VOLUME_SURGE_COMPARE_CANDLES = 6  # 对比前N根K线
BONUS_VOLUME_SURGE_MAX_PRICE_GAIN = 0.5 # 价格涨幅上限 (0.5 = 50%)
BONUS_VOLUME_SURGE_MIN_DELTA = 500      # 成交额异动最低金额 ($)

# --- 限制条件: 加分前必查 (v18 新增, v19 币龄分层) ---
# 任一条件不满足直接拒绝, 不进入加分计算
# 价格限制:
#   - 近1~4轮整体涨幅均≤50% (币龄≤1h时≤100%, 近N轮=当前价相对N轮前价格)
#   - 近1~8轮整体跌幅均≤15%
#   - h1价格变化有数据时: 涨幅≤50% (币龄≤1h时≤100%) 且 跌幅≤15%
#   - 快照最高价到当前价跌幅≤50%
LIMIT_PRICE_MAX_GAIN_ROUNDS = 4         # 检查近1~N轮整体涨幅
LIMIT_PRICE_MAX_GAIN_PCT = 0.50         # 整体涨幅上限 (50%, 币龄≤1h时实际为100%)
LIMIT_PRICE_MAX_DROP_ROUNDS = 8         # 检查近1~N轮整体跌幅
LIMIT_PRICE_MAX_DROP_PCT = 0.15         # 整体跌幅上限 (15%)
LIMIT_H1_MAX_GAIN_PCT = 0.50            # h1涨幅上限 (50%, 币龄≤1h时实际为100%)
LIMIT_H1_MAX_DROP_PCT = 0.15            # h1跌幅上限 (15%)
LIMIT_PEAK_DROP_MAX_PCT = 0.50          # 最高价到当前价跌幅上限 (50%)
# 进度限制 (加减法, 非乘除法):
#   - 近1~4轮整体进度涨幅均≤50个百分点
#   - 近1~8轮整体进度跌幅均≤15个百分点
LIMIT_PROGRESS_MAX_GAIN_ROUNDS = 4      # 检查近1~N轮整体进度涨幅
...
# 保留常量供历史代码兼容, 但不再使用
BONUS_H1_MIN_CHANGE = -0.20             # h1 最小变化 (-20%) [已废弃]
BONUS_H1_MAX_CHANGE = 1.0               # h1 最大变化 (100%) [已废弃]

# --- 加分标签: 防追高检查 (已废弃, 由限制条件替代) ---
# 保留常量供历史代码兼容, 但不再使用
BONUS_RECENT_MAX_3ROUND_GAIN = 1.0      # 近3轮总涨幅上限 (1.0 = 100%) [已废弃]
BONUS_RECENT_MIN_ROUNDS = 2             # 检查近N轮到近2轮 [已废弃]
BONUS_RECENT_MAX_ROUNDS = 8             # 检查近N轮到近2轮 [已废弃]
...
FILTER_MAX_GAIN_PREV2 = 1.0              # 相对前前根K线开盘价最大涨幅 (100%)
# 最高价到当前价格跌幅不大于 70%
FILTER_MAX_DRAWDOWN_FROM_HIGH = 0.70     # 从最高价最大回撤 (70%)

# --- 加分标签: 流动性异动 (liquidity surge, 两轮扫描之间) ---
# 要求: 两轮扫描之间流动性增加>=1k且流动性增长>=5%, 价格上涨且涨幅≤50%
BONUS_LIQ_SURGE_MIN_DELTA = 1000        # 流动性异动最低金额 ($)
BONUS_LIQ_SURGE_MIN_PCT = 0.05          # 流动性增长比例 (5%)
BONUS_LIQ_SURGE_MAX_PRICE_GAIN = 0.5    # 价格涨幅上限 (0.5 = 50%)

# --- 加分标签: 进度异动 (progress surge, 仅未毕业) ---
# 要求: 两轮扫描之间进度涨了10个百分点(加减法) + 价格上涨且涨幅≤50%
BONUS_PROGRESS_SURGE_MIN_DELTA = 0.10   # 进度增量下限 (10个百分点)
BONUS_PROGRESS_SURGE_MAX_PRICE_GAIN = 0.5  # 价格涨幅上限 (0.5 = 50%)

# --- 加分标签: 价格异动 (price surge) ---
# 要求: 两轮扫描之间价格增长>=10%且<=50%
BONUS_PRICE_SURGE_MIN_GAIN = 0.10    # 价格增长下限 (10%)
BONUS_PRICE_SURGE_MAX_GAIN = 0.5     # 价格增长上限 (50%)

# --- 加分标签权重 (只保留硬指标加分) ---
BONUS_WEIGHT_CONSOLIDATION = 1   # 小涨跌不动
BONUS_WEIGHT_VOLUME_SURGE = 1    # 成交额异动
BONUS_WEIGHT_LIQ_SURGE = 1       # 流动性异动
BONUS_WEIGHT_PROGRESS_SURGE = 1  # 进度异动
BONUS_WEIGHT_PRICE_SURGE = 1     # 价格异动
...
# 已确认的诈骗代币, 直接拦截
# 注意: 这个是硬编码的初始黑名单, 运行时会从 queue.json 的 tokenBlacklist 字段加载更多地址
# 每轮扫描会自动清理币龄>48h的地址
TOKEN_BLACKLIST = {
...
ELIM_LOW_HOLDERS_AGE_HOURS = 0.25
ELIM_LOW_HOLDERS_MIN = 5
# 队列分阶段收紧；先在早期淘汰低活跃币，减少后续外部 API 查询量。
ELIM_HOLDER_TIERS = [
    (0.25, 2),
    (0.5, 5),
    (1.0, 10),
...
def load_queue() -> dict:
    ...
    try:
        if QUEUE_FILE.exists():
            with open(QUEUE_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            # 迁移: 旧版 pushed 列表 → 新版 pushedMap 字典
            if "pushed" in data and "pushedMap" not in data:
                pushed_list = data["pushed"]
                data["pushedMap"] = {}
                now_ms = int(time.time() * 1000)
                for addr in pushed_list:
                    # 旧列表没有时间戳, 统一设为当前时间
                    data["pushedMap"][addr] = now_ms
                log.info("迁移 pushed 列表: %d 个地址 → pushedMap", len(pushed_list))
            # 迁移: 确保 qualityShownMap 存在
            if "qualityShownMap" not in data:
                data["qualityShownMap"] = {}
                log.info("初始化 qualityShownMap")
    ...
    return {"tokens": [], "eliminated": [], "lastBlock": 0, "lastScanTime": 0,
            "nameIndex": {}, "pushedMap": {}, "qualityShownMap": {}}
...
def save_queue(queue: dict):
    ...
    # 清理过期的 tokens（超过 48 小时）
    tokens = queue.get("tokens", [])
    if tokens:
        filtered_tokens = []
        for t in tokens:
            created_at = t.get("createdAt", 0)
            if created_at > 0 and now_ms - created_at > max_age_ms:
                continue  # 跳过过期代币
            filtered_tokens.append(t)
        queue["tokens"] = filtered_tokens
        removed_count = len(tokens) - len(filtered_tokens)
        if removed_count > 0:
            log.debug("清理 %d 个过期代币（>48h）", removed_count)
...
def update_deployer_reputation(queue_state: dict,
    ...
    # 更新开发者黑名单集合 (供 admission_filter 使用)
...
def _save_smart_money_file(data: dict):
    """保存聪明钱地址文件"""
    if 'SMART_MONEY_FILE' in globals():
        SMART_MONEY_FILE.parent.mkdir(exist_ok=True)
...
def merge_price_data(ds_data: dict[str, dict], gt_data: dict[str, dict]) -> dict[str, dict]:
    """
    合并 DexScreener 和 GeckoTerminal 数据
    
    优先使用 DexScreener 数据，当字段为空/零时用 GeckoTerminal 补充
    特别针对 flap 代币缺少 h1 数据的情况
    """
    result = dict(ds_data)  # 复制 DS 数据
    
    for addr, gt_info in gt_data.items():
        if addr not in result:
            ...
            # 补充 h1 涨跌幅 (flap 代币普遍缺这个)
            if ds_info.get("priceChangeH1", 0) == 0 and gt_info.get("priceChangeH1", 0) != 0:
                ds_info["priceChangeH1"] = gt_info["priceChangeH1"]
            
            # 补充 m5 涨跌幅
            if ds_info.get("priceChangeM5", 0) == 0 and gt_info.get("priceChangeM5", 0) != 0:
                ds_info["priceChangeM5"] = gt_info["priceChangeM5"]
            
            # 补充买卖笔数 (h1)
            if ds_info.get("buysH1", 0) == 0 and gt_info.get("buysH1", 0) != 0:
                ds_info["buysH1"] = gt_info["buysH1"]
                ds_info["sellsH1"] = gt_info["sellsH1"]
            
            # 补充交易量 (h1)
            if ds_info.get("volumeH1", 0) == 0 and gt_info.get("volumeH1", 0) != 0:
                ds_info["volumeH1"] = gt_info["volumeH1"]
            
            # 补充流动性 (已毕业代币可能 DS 返回 0)
            if ds_info.get("liquidity", 0) == 0 and gt_info.get("liquidity", 0) != 0:
                ds_info["liquidity"] = gt_info["liquidity"]
            
            # 补充价格 (兜底)
            if ds_info.get("price", 0) == 0 and gt_info.get("price", 0) != 0:
                ds_info["price"] = gt_info["price"]
    ...
    for t in fresh:
        ...
        if is_flap:
            ...
            if flap_meta is not None:
                ...
                # flap.sh 页面抓取失败, 继续 (社交仅供展示)
                log.warning("flap 社交抓取失败: %s", t["address"][:16])
                social_links = {}
                social_count = 0
                social_desc = ""
            flap_detail = {
...
def elimination_check(queue: list[dict], now_ms: int,
    ...
    if pre_elim_count > 0:
        log.info("淘汰: 本地预淘汰 %d 个", pre_elim_count)
    ...
    for t in pre_filtered:
        ...
        # 保留最近 25 轮 (用于币龄>24h代币的成交额差分法计算)
        # 差分法需要: 当前15分钟成交额 = volume24h差分 + 24轮前的快照成交额
        t["volumeHistory"] = vol_hist[-25:]
...
def _check_price_limit(price_hist: list[float], price_change_h1: float = None,
                       peak_price: float = 0, age_hours: float = 0) -> tuple[bool, str]:
    """
    限制条件: 价格限制检查 (v18 新增, v19 币龄分层)
    
    条件:
      - 近1~4轮整体涨幅均≤50% (币龄≤1h时≤100%)
      - 近1~8轮整体跌幅均≤15%
      - h1价格变化有数据时: 涨幅≤50% (币龄≤1h时≤100%) 且 跌幅≤15%
      - 快照最高价到当前价跌幅≤50%
    ...
    返回: (是否通过, 失败原因)
    """
    current_price = price_hist[-1] if price_hist else 0
    if current_price <= 0:
        return False, "当前价格无效"
    
    # 币龄≤1h时代币涨幅限制放宽到100%
    max_gain_pct = 1.0 if age_hours <= 1.0 else LIMIT_PRICE_MAX_GAIN_PCT
    h1_max_gain_pct = 1.0 if age_hours <= 1.0 else LIMIT_H1_MAX_GAIN_PCT
    
    # 1. h1价格变化检查 (有数据时必查)
    if price_change_h1 is not None:
        h1_pct = price_change_h1 / 100.0  # DexScreener 返回的是百分比值
        if h1_pct > h1_max_gain_pct:
            return False, f"h1涨幅{h1_pct*100:.1f}%>{h1_max_gain_pct*100:.0f}%"
        if h1_pct < -LIMIT_H1_MAX_DROP_PCT:
            return False, f"h1跌幅{abs(h1_pct)*100:.1f}%>{LIMIT_H1_MAX_DROP_PCT*100:.0f}%"
    
    # 2. 快照最高价到当前价跌幅检查
    if peak_price > 0:
        drop_from_peak = (peak_price - current_price) / peak_price
        if drop_from_peak > LIMIT_PEAK_DROP_MAX_PCT:
            return False, f"峰值跌幅{drop_from_peak*100:.1f}%>{LIMIT_PEAK_DROP_MAX_PCT*100:.0f}%"
    
    if not price_hist or len(price_hist) < 2:
        return True, ""  # 数据不足, 放行
    
    # 3. 近1~4轮整体涨幅检查 (币龄≤1h时≤100%, 否则≤50%)
    for n in range(1, min(LIMIT_PRICE_MAX_GAIN_ROUNDS, len(price_hist)) + 1):
        if n >= len(price_hist):
            break
        price_n_ago = price_hist[-(n + 1)]
        if price_n_ago > 0:
            gain_n_rounds = (current_price - price_n_ago) / price_n_ago
            if gain_n_rounds > max_gain_pct:
                return False, f"近{n}轮涨幅{gain_n_rounds*100:.1f}%>{max_gain_pct*100:.0f}%"
    
    # 4. 近1~8轮整体跌幅检查 (均不能超过15%)
    for n in range(1, min(LIMIT_PRICE_MAX_DROP_ROUNDS, len(price_hist)) + 1):
        if n >= len(price_hist):
            break
        price_n_ago = price_hist[-(n + 1)]
        if price_n_ago > 0:
            drop_n_rounds = (price_n_ago - current_price) / price_n_ago
            if drop_n_rounds > LIMIT_PRICE_MAX_DROP_PCT:
                return False, f"近{n}轮跌幅{drop_n_rounds*100:.1f}%>{LIMIT_PRICE_MAX_DROP_PCT*100:.0f}%"
    
    return True, ""


def _check_progress_limit(prog_hist: list[float]) -> tuple[bool, str]:
    """
    限制条件: 进度限制检查 (v18 新增)
    ...
    返回: (是否通过, 失败原因)
    """
    if not prog_hist or len(prog_hist) < 2:
        return True, ""  # 数据不足, 放行
    
    current_progress = prog_hist[-1]
    
    # 1. 近1~4轮整体进度涨幅检查 (加减法, 均不能超过50个百分点)
    for n in range(1, min(LIMIT_PROGRESS_MAX_GAIN_ROUNDS, len(prog_hist)) + 1):
        if n >= len(prog_hist):
            break
        prog_n_ago = prog_hist[-(n + 1)]
        gain_n_rounds = current_progress - prog_n_ago  # 加减法
        if gain_n_rounds > LIMIT_PROGRESS_MAX_GAIN_PCT:
            return False, f"近{n}轮进度涨{gain_n_rounds*100:.1f}pp>{LIMIT_PROGRESS_MAX_GAIN_PCT*100:.0f}pp"
    
    # 2. 近1~8轮整体进度跌幅检查 (加减法, 均不能超过15个百分点)
    for n in range(1, min(LIMIT_PROGRESS_MAX_DROP_ROUNDS, len(prog_hist)) + 1):
        if n >= len(prog_hist):
            break
        prog_n_ago = prog_hist[-(n + 1)]
        drop_n_rounds = prog_n_ago - current_progress  # 加减法
        if drop_n_rounds > LIMIT_PROGRESS_MAX_DROP_PCT:
            return False, f"近{n}轮进度跌{drop_n_rounds*100:.1f}pp>{LIMIT_PROGRESS_MAX_DROP_PCT*100:.0f}pp"
...
def _quality_key_hold_metrics(t: dict) -> dict:
    ...
    return {
        ...
        "key_hold_cross_pct": min(kol_pct, smart_pct),
    }
...
def _quality_bonus_tags(t: dict) -> tuple[int, list[str], dict]:
    ...
    # 1. 进度异动: 未毕业代币本轮 - 上轮 >= 15个百分点
    if not is_graduated and len(prog_hist) >= 2:
        prog_delta = prog_hist[-1] - prog_hist[-2]
        metrics["progress_delta"] = prog_delta
    ...
    # 4. 价格异动: 本轮 >= 上轮 * 1.2
    if len(price_hist) >= 2:
        price_prev = price_hist[-2]
        price_cur = price_hist[-1]
        if price_prev > 0 and price_cur > 0:
            price_gain = price_cur / price_prev - 1
            metrics["price_gain"] = price_gain
...
def tag_filter_legacy_v18(candidates: list[dict], now_ms: int,
    ...
    K 线防线 (最后防线, 只对通过加分标签的代币执行):
      ...
      - 最高价到当前价格跌幅不大于 70%
      - 假阳线检测: 第一根K线涨幅巨大实体柱、影线短、后续K线无振幅 → 诈骗币排除

    返回: (精筛结果列表, 空列表)
      - 精筛结果按加分总分降序排列
      - 无K线数据的代币标记 _kline_missing=True (仅记录, 不影响推送)
    """
    results = []
    ...
    for t in candidates:
        ...
        # ==================== 限制条件检查 (v18 新增, 加分前必查) ====================
        limit_passed, limit_reason = _check_limit_conditions(t)
        if not limit_passed:
        ...
        # --- 加分: 成交额异动 ---
        vol_hist = t.get("volumeHistory", [])
        if len(vol_hist) >= 2 and len(price_hist) >= 2 and price_hist[-2] > 0:
            is_bullish_candle = price_hist[-1] > price_hist[-2]
            vol_diff = vol_hist[-1] - vol_hist[-2]
            
            if age_hours >= 24:
                if len(vol_hist) >= 25:
                    vol_24_rounds_ago = vol_hist[-25] - vol_hist[-26] if len(vol_hist) >= 26 else None
                    vol_cur = vol_diff + vol_24_rounds_ago if vol_24_rounds_ago else None
                else:
                    vol_cur = None
            else:
                vol_cur = vol_diff
            
            if vol_cur is not None and vol_cur >= 0:
                prev_vols = []
                for i in range(2, min(len(vol_hist), BONUS_VOLUME_SURGE_COMPARE_CANDLES + 2)):
                    if vol_hist[-i] > vol_hist[-i-1]:
                        prev_vols.append(vol_hist[-i] - vol_hist[-i-1])
                if prev_vols:
                    vol_prev_sum = sum(prev_vols)
                    n = len(prev_vols)
                    vol_threshold = vol_prev_sum / n * BONUS_VOLUME_SURGE_COMPARE_CANDLES
                    price_gain = (price_hist[-1] - price_hist[-2]) / price_hist[-2]
                    if (is_bullish_candle and vol_threshold > 0
                            and vol_cur >= vol_threshold
                        ...
                        ratio = vol_cur / vol_threshold
                        bonus_tags.append(f"成交额异动({ratio:.1f}x)")

        # --- 加分: 流动性异动 (仅已毕业) ---
        if is_graduated:
            liq_hist = t.get("liquidityHistory", [])
            if len(liq_hist) >= 2 and len(price_hist) >= 2 and price_hist[-2] > 0:
                liq_cur = liq_hist[-1]
                liq_prev = liq_hist[-2]
                liq_delta = liq_cur - liq_prev
                liq_pct = liq_delta / liq_prev if liq_prev > 0 else 0
                price_gain = (price_hist[-1] - price_hist[-2]) / price_hist[-2]
                if (price_gain > 0 and price_gain <= BONUS_LIQ_SURGE_MAX_PRICE_GAIN
                        and liq_delta >= BONUS_LIQ_SURGE_MIN_DELTA
                        and liq_pct >= BONUS_LIQ_SURGE_MIN_PCT):
        ...
        if not is_graduated:
            prog_hist = t.get("progressHistory", [])
            if len(prog_hist) >= 2 and len(price_hist) >= 2 and price_hist[-2] > 0:
                prog_delta = prog_hist[-1] - prog_hist[-2]
                price_gain = (price_hist[-1] - price_hist[-2]) / price_hist[-2]
                if (prog_delta >= BONUS_PROGRESS_SURGE_MIN_DELTA
                        and price_gain > 0
                        and price_gain <= BONUS_PROGRESS_SURGE_MAX_PRICE_GAIN):
        ...
        if is_graduated and len(price_hist) >= 2 and price_hist[-2] > 0:
            ...
            if (price_gain >= BONUS_PRICE_SURGE_MIN_GAIN
                ...
                bonus_score += BONUS_WEIGHT_PRICE_SURGE
                bonus_tags.append(f"价格异动(+{price_gain*100:.0f}%)")
        ...
        # ==================== K 线防线检查 (最后防线) ====================
        gt_pool_addr = t.get("gtPoolAddress")
        passed, fail_reason, kline_data = check_kline_defense(
            addr, gt_pool_addr, current_price, progress, age_hours
        )
        if not passed:
            log.info("K线防线: ✗ %s — %s", name, fail_reason)
            continue

        # 记录是否有K线数据 (用于推送筛选)
        if kline_data and kline_data.get("_kline_missing"):
            t["_kline_missing"] = True
        elif kline_data:
            t["_kline_highest"] = kline_data.get("highest")
            t["_kline_lowest"] = kline_data.get("lowest")
            t["_kline_count"] = kline_data.get("count")
...
def tag_filter(candidates: list[dict], now_ms: int,
    ...
    for t in candidates:
        ...
        t["_age_hours"] = age_hours
        t["_min_holders"] = _age_tier_match(age_hours, TAG_HOLDERS_TIERS)
        t["_quality_metrics"] = base_metrics
        t["isGraduated"] = is_graduated
        results.append(t)
...
def format_message(results: list[dict]) -> str:
    ...
    for i, item in enumerate(results, 1):
        ...
        is_graduated = item.get("progress", 0) >= 1.0 or item.get("isGraduated", False)

        lines.append(f"### #{i} {symbol} ({name})")
        # 标签制统一通道，不再区分毕业通道/火箭通道
        if is_graduated:
            lines.append("🎓 已毕业")
        ...
        if is_graduated and liquidity > 0:
            lines.append(f"💵 流动性: ${liquidity:,.0f}")
        ...
        # 价格变化: 显示所有时间段 (m5/h1/h6/h24)
        pc_m5 = item.get("priceChangeM5", 0)
        pc_h1 = item.get("priceChangeH1", 0)
        pc_h6 = item.get("priceChangeH6", 0)
        pc_h24 = item.get("priceChangeH24", 0)
        if pc_m5 != 0 or pc_h1 != 0 or pc_h6 != 0 or pc_h24 != 0:
            pc_parts = []
            if pc_m5 != 0:
                pc_parts.append(f"5m {pc_m5:+.1f}%")
            if pc_h1 != 0:
                pc_parts.append(f"1h {pc_h1:+.1f}%")
            if pc_h6 != 0:
                pc_parts.append(f"6h {pc_h6:+.1f}%")
            if pc_h24 != 0:
                pc_parts.append(f"24h {pc_h24:+.1f}%")
            if pc_parts:
                lines.append(f"📉 涨跌: {' / '.join(pc_parts)}")
        ...
        key_hold = _quality_key_hold_metrics(item)
        lines.append(
            "🧩 关键持仓: "
            f"开发者 {key_hold['dev_hold_pct']:.2f}% / "
            f"KOL {key_hold['kol_hold_pct']:.2f}% / "
            f"聪明钱 {key_hold['smart_money_hold_pct']:.2f}%"
        )
        # 开发者行为
    ...
    try:
        ...
        for t in tokens:
            result.append({
                ...
                "createdAt": t.get("createdAt", t.get("created_at", 0)),
...
def scan_once(cfg: dict) -> dict:
    ...
    # 从持久化数据恢复诈骗代币黑名单, 并清理币龄>48h的地址
    _tbl = queue_state.get("tokenBlacklist", {})
    cleaned_count = 0
    for addr in list(_tbl.keys()):
        added_at = _tbl[addr].get("addedAt", 0)
        if now_ms - added_at > MAX_AGE_HOURS * 3600 * 1000:
            del _tbl[addr]
            cleaned_count += 1
    if cleaned_count:
        log.info("诈骗代币黑名单: 清理 %d 个币龄>%dh的地址", cleaned_count, MAX_AGE_HOURS)
    # 合并硬编码黑名单和持久化黑名单
    TOKEN_BLACKLIST.update(_tbl.keys())
    if _tbl:
        log.info("诈骗代币黑名单: 加载 %d 个地址", len(_tbl))
    ...
    # 前端展示去重: 同一个代币只在前端精筛结果里展示第一次通过的
    quality_shown_map = queue_state.get("qualityShownMap", {})
    new_quality = []
    shown_repeat_count = 0
    for t in quality_results:
        addr = t.get("address", "")
        if addr in quality_shown_map:
            shown_repeat_count += 1
            log.info("前端展示去重: 跳过 %s (已展示过)", t.get("name") or addr[:16])
        else:
            new_quality.append(t)
    if shown_repeat_count > 0:
        log.info("前端展示去重: 过滤 %d 个已展示过的代币, 剩余 %d 个新代币",
                 shown_repeat_count, len(new_quality))
    # 前端展示用这个去重后的结果
    quality_results_for_frontend = new_quality

    # 钉钉推送去重: 同一个代币只在钉钉推一次 (和前端展示独立)
    pushed_map = queue_state.get("pushedMap", {})
    quality_results_for_dingding = []
    dingding_repeat_count = 0
    for t in quality_results_for_frontend:
        addr = t.get("address", "")
        if addr in pushed_map:
            dingding_repeat_count += 1
            log.info("钉钉推送去重: 跳过 %s (已推送过)", t.get("name") or addr[:16])
        else:
            quality_results_for_dingding.append(t)
    if dingding_repeat_count > 0:
        log.info("钉钉推送去重: 过滤 %d 个已推送过的代币, 剩余 %d 个新代币",
                 dingding_repeat_count, len(quality_results_for_dingding))
    ...
    # 统计信息
    queue_state["newDiscovered"] = len(new_on_chain)
    queue_state["newAdmitted"] = len(admitted)
    queue_state["eliminatedCount"] = len(eliminated)
    ...
    # 只保留近48小时的记录
    cutoff_time = now_ms - (48 * 60 * 60 * 1000)
    eliminated_history = [
        entry for entry in eliminated_history
        if entry["time"] >= cutoff_time
    ]
    queue_state["eliminatedHistory"] = eliminated_history
    # 计算近48小时累计淘汰
    queue_state["eliminatedTotal48h"] = sum(
        entry["count"] for entry in eliminated_history
    )
    
    # 记录本轮前端展示的代币到 qualityShownMap (去重用, 在保存/输出之前记录)
    quality_shown_map = queue_state.get("qualityShownMap", {})
    for t in quality_results_for_frontend:
        addr = t.get("address", "")
        if addr and addr not in quality_shown_map:
            quality_shown_map[addr] = now_ms
    # 清理币龄超过 48 小时的记录 (避免 qualityShownMap 无限增长)
    # 被淘汰超过 48 小时的代币不会再进入精筛流程，可以清理
    cutoff_time = now_ms - (48 * 60 * 60 * 1000)
    expired_addrs = [addr for addr, ts in quality_shown_map.items() if ts < cutoff_time]
    for addr in expired_addrs:
        del quality_shown_map[addr]
    queue_state["qualityShownMap"] = quality_shown_map

    # 记录本轮钉钉推送的代币到 pushedMap (去重用, 在推送/买入之前记录)
    pushed_map = queue_state.get("pushedMap", {})
    for t in quality_results_for_dingding:
        addr = t.get("address", "")
        if addr and addr not in pushed_map:
            pushed_map[addr] = now_ms
    # 同样清理超过 48 小时的记录
    expired_addrs = [addr for addr, ts in pushed_map.items() if ts < cutoff_time]
    for addr in expired_addrs:
        del pushed_map[addr]
    queue_state["pushedMap"] = pushed_map
    save_queue(queue_state)

    # 不再合并 token_scanner 的精筛结果, 只用本地扫出的币
    ...
    trading_enabled = _HAS_TRADER and trading_cfg.get("enabled", False)
    ...
    if trading_enabled and len(bonus_filtered) > 0:
        ...
        for t in bonus_filtered:
            ...
            if in_cd:
                log.info("推送过滤: 跳过 %s (%s)", name, cd_reason)
                cooldown_count += 1
                continue
    ...
    if cooldown_count > 0:
        log.info("冷却期过滤: %d 个在冷却期内 (不推送)", cooldown_count)

    filtered = final_filtered[:max_push]
    msg = format_message(filtered)
    log.info("推送 %d 个代币", len(filtered))
    ...
    if trading_enabled:
        ...
        try:
            ...
            # 推送未开仓原因 (排除已持仓和冷却期, 这些已在推送前过滤)
            if skipped:
                from trader import notify_buy_skipped
                notify_buy_skipped(cfg, skipped)
        except Exception as e:
...
def output_scan_json(queue_state: dict, eliminated_this_round: list = None,
