...
4. 精筛 (瞬时)
   横盘不跌 + 微微上涨
   → 近 24 小时高低跌幅不大于 30%，当前价比前前轮高 10%~20%
...
| #   | 条件        | 说明             |
| --- | ----------- | ---------------- |
| 1   | 横盘不跌    | 近 24 小时内最高价到最低价的跌幅不大于 30%；历史不足 24 小时时使用已记录价格 |
| 2   | 微微上涨    | 当前价 ≥ 前前轮价格 × 1.1，且当前价 ≤ 前前轮价格 × 1.2 |

旧版“限制条件 + 基础标签 + 加分标签 + K线防线”策略已在代码中备份为 `tag_filter_legacy_v18`, 当前不参与开仓。当前策略不再使用原先的基础标签和加分标签。
...
| #   | 策略            | 条件                                                            | 说明                                 |
| --- | --------------- | --------------------------------------------------------------- | ------------------------------------ |
| 1   | 清仓止盈        | 盈利 ≥ 100% 卖出全部剩余持仓                                    | 翻倍后落袋                           |
| 2   | 固定止损        | 亏损 ≥ 20%                                                      | 控制单仓亏损                         |
| 3   | 时间止损        | 买入 30 分钟内最高盈利没到 +15%                                 | 动能不足就退出                       |
| 5   | 重买冷却        | 盈利平仓后 6h, 亏损平仓后 12h                                   | 避免反复追同一个币                   |

旧版回撤/中点/10%分段止盈策略已在代码中备份为 `check_sell_conditions_legacy_v18`, 当前不参与卖出。
...
| `trading.take_profit_full_pct` | 100    | 清仓止盈阈值（%）   |
| `trading.time_stop_minutes` | 30     | 时间止损等待分钟    |
| `trading.time_stop_min_profit_pct` | 15     | 时间止损最低最高盈利（%） |
| `trading.stop_loss_pct`  | -20    | 止损阈值（%）       |
