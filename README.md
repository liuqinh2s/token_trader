...
4. 精筛 (瞬时)
   限制条件 + 加分标签 + K线防线
   → 每个维度按币龄精细化分层, 币龄越长要求越高, 高优先级优先买入
...
> 注意：由于 GeckoTerminal 的限流非常严重，而K线数据又十分宝贵，所以只在通过加分项后才请求K线数据，且 GeckoTerminal 的其他数据接口均放弃使用。
...
#### 6. GeckoTerminal OHLCV — `GET /api/v2/networks/bsc/pools/{address}/ohlcv/minute?aggregate=15&limit=48`

```json
{
  "data": {
    "type": "ohlcv",
    "id": "bsc/0xd001bc921198b70b4631ea75ff2fb744f23b4444",
    "attributes": {
      "ohlcv_list": [
        [
          "1736488800000",
...
candles = [
    [1736488800000, 0.00002310, 0.00002350, 0.00002280, 0.00002320, 1234.56],
    [1736489700000, 0.00002320, 0.00002400, 0.00002300, 0.00002380, 2345.67],
...
2. GeckoTerminal 15min K线最高价修正（仅对通过加分项后的代币，补充快照间隔内的冲高遗漏）
...
| #   | 条件                                          | 说明                      |
| --- | --------------------------------------------- | ------------------------- |
| 1   | 近1~4轮整体涨幅均≤50%                        | 近N轮=当前价相对N轮前价格 |
| 2   | h1价格变化有数据时: 涨幅≤50%                 | DexScreener 1小时数据     |
| 3   | 近1~8轮整体跌幅均≤15%                        | 防止已崩盘币              |
| 4   | h1价格变化有数据时: 跌幅≤15%                 | DexScreener 1小时数据     |
| 5   | 快照最高价到当前价跌幅≤50%                   | 防止已大涨后追高          |
| 6   | 近1~4轮整体进度涨幅均≤50个百分点              | 防止进度暴涨              |
| 7   | 近1~8轮整体进度跌幅均≤15个百分点             | 防止进度已崩盘            |

### 基础标签（全部满足，AND）

| 标签     | 条件                 | 说明                       |
| -------- | -------------------- | -------------------------- |
| 进度合格 | 当前进度 ≥ 20%       | 所有未毕业代币必须满足     |
| 波动充足 | 价格历史存在≥10%涨幅 | 过滤完全没动能的存活币     |

### 加分标签（通过限制条件后，至少一项才开仓）

| 加分项     | 条件                                                   | 权重 | 说明                   |
| ---------- | ------------------------------------------------------ | ---- | ---------------------- |
| 小涨跌不动 | 持币数≥10 且 最低→最高涨幅≤3.5倍 且 最高→最低跌幅≤55%, 横盘≥3h且≤8h | +1   | 横盘整理，蓄势待发     |
| 成交额异动 | 阳线 + 成交额≥前6根之和(不足6根归一化) + 增量≥500u     | +1   | 放量上涨，资金入场     |
| 流动性异动 | 仅已毕业, 两轮之间流动性增加≥$1k且增长≥5%, 价格上涨    | +1   | 流动性注入，项目方实力 |
| 进度异动   | 仅未毕业, 两轮之间进度涨≥10个百分点 + 价格上涨         | +1   | 进度突破，热度上升     |
| 价格异动   | 两轮扫描之间价格增长≥10%                               | +1   | 价格突破，动量确认     |

### K 线防线（最后防线，只对通过加分标签的代币执行）

| #   | 条件                                                          | 说明                          |
| --- | ------------------------------------------------------------- | ----------------------------- |
| 1   | 防追高: 当前价 ≤ 前一根K线开盘价的 50%，≤ 前前根开盘价的 100% | 避免追高                      |
| 2   | 最高价到当前价跌幅 ≤ 70%                                      | 排除已崩盘币                  |

> K 线数据通过 GeckoTerminal API 获取 15 分钟 K 线。无K线数据时跳过K线相关检查、直接放行，不影响精筛推送和开仓。通过了精筛，如果有K线数据的话，要在推送中说明：通过了K线防线。
...
| #   | 策略            | 条件                                                            | 说明                                 |
| --- | --------------- | --------------------------------------------------------------- | ------------------------------------ |
| 1   | 阶梯式回撤止盈  | 涨 15% 触发跟踪; 15%~30% 从最高价回撤 15% 卖出; ≥30% 中点止盈法 | 保住本金, 给好币更多空间, 大涨锁利润 |
| 2a  | 超期清仓 (亏损) | 持仓 ≥ 24h 且仍亏损                                             | 给了足够时间还亏就别等了             |
| 2b  | 超期清仓 (低效) | 持仓 ≥ 48h 且盈利 < 15%                                         | 资金效率太低，回收去抓下一个         |
| 3   | 兜底止损        | 亏损 ≥ 25%                                                      | 保住本金, 不给太多空间               |
...
| `trading.buy_fraction`   | 0.01   | 每仓金额占比        |
| `trading.min_buy_usd`    | 1      | 最小买入金额（USD） |
| `trading.max_buy_usd`    | 10     | 最大买入金额（USD） |
| `trading.tp_trigger_pct` | 15     | 止盈触发阈值（%）   |
| `trading.stop_loss_pct`  | -25    | 止损阈值（%）       |
