...
2. 入场筛 (~数秒)
   ...
   → 淘汰总量≠10亿 / 币龄>48h；存活队列要求至少 1 个社交链接
...
4. 精筛 (瞬时)
   币龄12~48h && 进度>=50% && 持币>=50 && 低价稳定缓涨 && Top10持仓<=20%
   → 全部条件 AND
...
| 1   | 价格从峰值回撤 70%+             | 暴跌弃盘 (当前价格<1e-7 视为 API 异常, 跳过) |
| 1b  | 单根K线跌幅 > 55%               | 过山车币 (暴涨后暴跌)                        |
| 2   | 流动性跌破 $100 (仅已毕业)      | 流动性枯竭                                   |
| 2b  | 没有关联社交媒体                | Twitter、Telegram、官网等链接至少存在一个    |
| 2c  | 持币数查询失败或为 0            | 不等待后续回填，直接淘汰                      |
| 3   | 持币地址阶梯不达标               | 1/2/4/8/12/24/36h 至少 5/10/20/35/50/75/100 |
| 3b  | 进度阶梯不达标                   | 同期至少 3%/8%/15%/30%/50%/65%/80%           |
| 3c  | 最高价 > 0.0001                  | 已永久失去精筛资格，提前清理                  |
| 4   | 进度从峰值跌 50 个百分点+        | 热度消退 (加减法)                            |
| 5   | 币龄 > 48h                      | 超出关注窗口                                 |
| 6   | 诈骗代币黑名单                   | 代币合约地址黑名单                           |
| 7   | 诈骗开发者黑名单                 | 开发者钱包地址黑名单                         |

> 注: 入场时允许社交数据暂缺，但队列存活必须至少有 1 个社交链接
...
| # | 条件 | 说明 |
| --- | --- | --- |
| 1 | 币龄 | 12–48h，至少完整观察 12 小时 |
| 2 | 进度 / 持币 | 进度 ≥50%，持币地址 ≥50 |
| 3 | 当前价 / 最高价 | 当前价 ≤0.00004，最高价 ≤0.0001 |
| 4 | 稳定性 | 峰值回撤 ≤50%；最近 12h 单轮波动 ≤20%，整体不走低且涨幅 ≤200% |
...
| `queue_recent_1h_only`  | false  | 紧急开关；开启后队列只保留币龄 1h 内代币。正常策略关闭，按 48h 分阶段淘汰 |
